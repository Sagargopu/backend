...
@router.get("/workers/by-profession/{profession_id}", response_model=List[schemas.Worker])
def read_workers_by_profession(profession_id: int, db: Session = Depends(get_db)):
    """Get all workers by profession ID"""
    workers = crud.get_workers_by_profession(db, profession_id=profession_id)
    return workers

@router.get("/workers/by-availability/{availability}", response_model=List[schemas.Worker])
...
@router.get("/workers/available", response_model=List[schemas.Worker])
...
@router.get("/projects/{project_id}/worker-history", response_model=List[schemas.WorkerProjectHistory])
def read_project_worker_histories(project_id: int, db: Session = Depends(get_db)):
    """Get all worker histories for a specific project"""
    histories = crud.get_project_worker_histories(db, project_id=project_id)
    return histories
